...
db.collection('users_donors').doc('users').collection('usersList').where('barangay', '==', '182').orderBy('accountCreation','desc').onSnapshot(snapshot => {
    let changes = snapshot.docChanges();
    ...
    changes.forEach(change => {
        ...
        db.collection('users_donors').doc('users').collection('usersList').where('barangay', '==', '182').onSnapshot(snap => {
            size = snap.size // will return the collection size
            document.getElementById("value").innerHTML = size;
            });

            db.collection('users_donors').doc('users').collection('usersList').where('barangay', '==', '182').where('status', '==', status_verified).onSnapshot(snap => {
            size = snap.size // will return the collection size
            document.getElementById("value1").innerHTML = size;
          });

          db.collection('users_donors').doc('users').collection('usersList').where('barangay', '==', '182').where('status', '==', status_nVerified).onSnapshot(snap => {
        size = snap.size // will return the collection size
