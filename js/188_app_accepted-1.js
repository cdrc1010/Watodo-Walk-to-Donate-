...
function renderCafe(doc) {
    ...
    cross.addEventListener('click', (e) => {
        e.stopPropagation();
        let id = e.target.parentElement.getAttribute('data-id');
        db.collection('donations').doc('188').collection('acceptedDonations').doc(id).delete();
    });
    ...
    view.addEventListener('click', (e) => {
        ...
        db.collection('donations').doc('188').collection('acceptedDonations').doc(id).get().then(doc => {
            const html = `
...
db.collection('donations').doc('188').collection('acceptedDonations').onSnapshot(snapshot => {
    let changes = snapshot.docChanges();
