    ...
    btnAccept.addEventListener('click', (e) => {
        ...
        db.collection('donations').doc('peerToPeer').collection('userDonations').doc(id).get().then(doc => {
            ...
            Email.send({
                ...
                Body : "Please check your donation"
            }).then(
...
db.collection('donations').doc('peerToPeer').collection('userDonations').orderBy('dateAdded').onSnapshot(snapshot => {
    let changes = snapshot.docChanges();
...
const status_accepted = "accepted";    
const status_declined = "declined"; 
db.collection('donations').doc('peerToPeer').collection('acceptedDonations').where('status', '==', status_accepted).get().then(snap => {
    size = snap.size // will return the collection size
    document.getElementById("value1").innerHTML = size;
  });
  
  db.collection('donations').doc('peerToPeer').collection('declinedDonations').where('status', '==', status_declined).get().then(snap => {
    size = snap.size // will return the collection size
    document.getElementById("value2").innerHTML = size;
  });

db.collection('donations').doc('peerToPeer').collection('userDonations').where('status', '==', status_pending).get().then(snap => {
size = snap.size // will return the collection size
