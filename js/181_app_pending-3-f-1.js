...
function renderCafe(doc) {
    ...
    btnAccept.addEventListener('click', (e) => {
        ...
        db.collection('donations').doc('181').collection('userDonations').doc(id).get().then(doc => {
            db.collection('donations').doc('181').collection('userDonations').doc(id).update({
                    status: 'accepted'
                });
                
                db.collection('donations').doc('181').collection('acceptedDonations').doc(id).set({
                ...
                return db.collection('donations').doc('181').collection('acceptedDonations').doc(id).get().then(doc => {
            ...
            db.collection('donations').doc('181').collection('userDonations').doc(id).delete();
        });
    ...
    btnDecline.addEventListener('click', (e) => {
        ...
        db.collection('donations').doc('181').collection('userDonations').doc(id).get().then(doc => {
            db.collection('donations').doc('181').collection('userDonations').doc(id).update({
                ...
                db.collection('donations').doc('181').collection('declinedDonations').doc(id).set({
                donorName:  doc.data().donorName,
                donorUID:  doc.data().donorUID,
                donationUID: doc.data().donationUID,
                barangay: doc.data().barangay,
                ...
                return db.collection('donations').doc('181').collection('declinedDonations').doc(id).get().then(doc => {
                    email = doc.data().donorEmail;
...
db.collection('donations').doc('181').collection('userDonations').where('status', '==', 'pending').orderBy('dateAdded').onSnapshot(snapshot => {
    let changes = snapshot.docChanges();
...
db.collection('donations').doc('181').collection('userDonations').where('status', '==', 'pending').onSnapshot(snap => {
    size = snap.size // will return the collection size
    document.getElementById("value").innerHTML = size;
  });
  
  db.collection('donations').doc('181').collection('acceptedDonations').where('status', '==', status_accepted).onSnapshot(snap => {
    size = snap.size // will return the collection size
    document.getElementById("value1").innerHTML = size;
  });

  db.collection('donations').doc('181').collection('declinedDonations').where('status', '==', status_declined).onSnapshot(snap => {
size = snap.size // will return the collection size
