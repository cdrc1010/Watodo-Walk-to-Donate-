...
function renderCafe(doc) {
    ...
    cross.addEventListener('click', (e) => {
        e.stopPropagation();
        let id = e.target.parentElement.getAttribute('data-id');
        db.collection('donations').doc('182').collection('declinedDonations').doc(id).delete();
    });
    ...
    view.addEventListener('click', (e) => {
        ...
        db.collection('donations').doc('182').collection('declinedDonations').doc(id).get().then(doc => {
            const html = `
...
db.collection('donations').doc('182').collection('declinedDonations').onSnapshot(snapshot => {
    let changes = snapshot.docChanges();
...
db.collection('donations').doc('182').collection('userDonations').where('status', '==', status_pending).get().then(snap => {
    size = snap.size // will return the collection size
    document.getElementById("value").innerHTML = size;
  });
  
  db.collection('donations').doc('182').collection('acceptedDonations').where('status', '==', status_accepted).get().then(snap => {
    size = snap.size // will return the collection size
    document.getElementById("value1").innerHTML = size;
  });

  db.collection('donations').doc('182').collection('declinedDonations').where('status', '==', status_declined).get().then(snap => {
size = snap.size // will return the collection size
