...
function renderCafe(doc) {
    ...
    let org_Name = document.createElement('span');
    ...
    // text content
    org_Name.textContent = doc.data().orgName;
    ...
    li.appendChild(org_Name);
    li.appendChild(acc_Created);
    li.appendChild(status);
    li.appendChild(cross);
    li.appendChild(view);
    statusValue = doc.data().status;
    ...
    btnAccept.addEventListener('click', (e) => {
        ...
        db.collection('users_donors').doc('donors').collection('donorsList').doc(id).update({
            status: 'verified'
        });
    });
    ...
    cross.addEventListener('click', (e) => {
        e.stopPropagation();
        let id = e.target.parentElement.getAttribute('data-id');
        db.collection('users_donors').doc('donors').collection('donorsList').doc(id).delete();
    });
    ...
    view.addEventListener('click', (e) => {
        ...
        db.collection('users_donors').doc('donors').collection('donorsList').doc(id).get().then(doc => {
            const html = `
        <div style="display:flex; line-height: 50px;"> 
        <div>
        Organization Name: ${doc.data().orgName}<br>
        Email: ${doc.data().orgEmail}<br>
...
db.collection('users_donors').doc('donors').collection('donorsList').where('barangay', '==', '178').onSnapshot(snapshot => {
    let changes = snapshot.docChanges();
    ...
    changes.forEach(change => {
        ...
        db.collection('users_donors').doc('donors').collection('donorsList').where('barangay', '==', '178').get().then(snap => {
            size = snap.size // will return the collection size
            document.getElementById("value").innerHTML = size;
            });

        db.collection('users_donors').doc('donors').collection('donorsList').where('barangay', '==', '178').where('status', '==', status_verified).get().then(snap => {
            size = snap.size // will return the collection size
            document.getElementById("value1").innerHTML = size;
          });

        db.collection('users_donors').doc('donors').collection('donorsList').where('barangay', '==', '178').where('status', '==', status_nVerified).get().then(snap => {
        size = snap.size // will return the collection size
