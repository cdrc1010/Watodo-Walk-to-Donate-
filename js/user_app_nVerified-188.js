...
db.collection('users_donors').doc('users').collection('usersList').where('barangay', '==', '188').onSnapshot(snapshot => {
    let changes = snapshot.docChanges();
    ...
    changes.forEach(change => {
        ...
        db.collection('users_donors').doc('users').collection('usersList').where('barangay', '==', '188').get().then(snap => {
            size = snap.size // will return the collection size
            document.getElementById("value").innerHTML = size;
            });

            db.collection('users_donors').doc('users').collection('usersList').where('barangay', '==', '188').where('status', '==', status_verified).get().then(snap => {
            size = snap.size // will return the collection size
            document.getElementById("value1").innerHTML = size;
          });

          db.collection('users_donors').doc('users').collection('usersList').where('barangay', '==', '188').where('status', '==', status_nVerified).get().then(snap => {
        size = snap.size // will return the collection size
