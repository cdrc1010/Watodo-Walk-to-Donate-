...
db.collection('users_donors').doc('donors').collection('donorsList').where('barangay', '==', '182').onSnapshot(snapshot => {
    let changes = snapshot.docChanges();
    ...
    changes.forEach(change => {
        ...
        db.collection('users_donors').doc('donors').collection('donorsList').where('barangay', '==', '182').get().then(snap => {
            size = snap.size // will return the collection size
            document.getElementById("value").innerHTML = size;
            });

        db.collection('users_donors').doc('donors').collection('donorsList').where('barangay', '==', '182').where('status', '==', status_verified).get().then(snap => {
            size = snap.size // will return the collection size
            document.getElementById("value1").innerHTML = size;
          });

        db.collection('users_donors').doc('donors').collection('donorsList').where('barangay', '==', '182').where('status', '==', status_nVerified).get().then(snap => {
        size = snap.size // will return the collection size
